...
public class ChC2{
    ...
    public static void main(String[] args) throws InterruptedException {
        String name;
        boolean isMan = true;
        System.out.println("Dobrý den, jaké je Vaše křestní jméno? (i s diakritikou prosím)");
        name = sc.nextLine();
        ...
        else if(name.equalsIgnoreCase("Vendy")){
            osloveni = "Vendy";
            isMan = false;
        }
        ...
        if(answer.equalsIgnoreCase("Ano")){
            for (int step = 1; step <= 4; step++){
                Clear(); Move(step, osloveni); TimeUnit.MILLISECONDS.sleep(220); 
                if (step == 4) step = 0;
            }   
        }else{
            doesntLike(isMan);
